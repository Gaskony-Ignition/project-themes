...
def build_project_json(version):
    return {
        "title": "Theme Installer",
        "description": (
            "Install or remove Gaskony's 9 curated Perspective gateway "
            "themes (glass-violet, glass-green, leather-dark, "
            "leather-light, finance-ledger, nord-dark, nord-light, "
            "industrial-dark, industrial-light) with one click -- no "
            "shell, no docker exec. Writes gateway config resources under "
            "com.inductiveautomation.perspective/themes/ and triggers a "
            "config scan. Parent-free, safe to delete after use. "
            "Generated by ignition-themes/"
            "build_installer.py. · v%s" % version
        ),
        "enabled": True,
        "inheritable": False,
        "parent": "",
...
def build_themepack_code(themes, version):
    ...
    embedding all 9 themes' file contents plus install/uninstall/status.
    ...
    """
    lines = []
    lines.append('"""themepack -- embeds Gaskony\'s 9 curated Perspective')
            ...
            "children": [
                {
                    ...
                    "children": [
                        ...
                        },
                        {
                            ...
                            "props": {
                                "text": (
                                    "Installs Gaskony's 9 curated Perspective gateway "
                                    "themes as config resources. Safe to re-run; safe "
                                    "to delete this project afterwards. v%s" % version
                                ),
                                "style": {
                                    "fontSize": "13px",
...
def main():
    ...
    # project.json
    write_json(os.path.join(PROJECT_DIR, "project.json"), build_project_json(version))
