...
4. The page's table updates itself: the Status column flips to "Installed" for all 9 rows once the scan lands (a couple of seconds). Cross-check with step 3 below if you want the `curl` version too.
...
Generated from `ignition-themes/` — `mapping.py` (the token → theme-variable table) and `build_theme.py` (the generator). Not part of that repo's packs/families source-of-truth pipeline, and not wired into its `tools/build_all.py`. See that repo's `README.md` for the full generator documentation, including per-theme derivation notes and the `glass-green` colour-correction tweak.
