...
Generated by `mapping.py` (the token → theme-variable table) and `build_theme.py`, at <https://github.com/Gaskony-Ignition/project-themes>. That repo's `docs/INTERNALS.md` carries the generator documentation, including per-theme derivation notes, the style-class contract and the `glass-green` colour correction.
